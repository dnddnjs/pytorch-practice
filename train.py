...
def train_controller(child, controller, running_reward, entropy_seq, log_prob_seq):
    ...
    for batch_idx, (inputs, targets) in enumerate(valid_loader):
        ...
        entropy_bonus = torch.cat(entropy_seq, dim=0).sum()
...
def test_final_model(model):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            ...
            print('epoch : [{}/{}]| loss: {:.3f} | acc: {:.3f}'.format(epoch, batch_idx,
                                                                       len(test_loader), test_loss / (batch_idx + 1),
                                                                       100 * correct / total))
...
def main(controller_model, cosine_annealing_scheduler):
    ...
    for epoch in range(310):
        ...
        if epoch < 310 - 1:
            controller_model, running_reward = train_controller(model, controller_model, running_reward, entropy_seq,
                                                                log_prob_seq)
